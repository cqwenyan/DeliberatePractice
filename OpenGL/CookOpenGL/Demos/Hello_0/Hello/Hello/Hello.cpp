...
//};
//// VBO(Vertex buffer object)顶点缓存对象 float数据（ 坐标、UV、法线、颜色、索引）
...
//void InitData()
//{
//	// 创建VBO需要三步，1.glGenBuffers()生成新缓存对象；2.glBindBuffer()绑定缓存对象；3.使用glBufferData()将顶点数据拷贝到缓存对象中；
...
//	// GL_STATIC_DRAW[STATIC表示数据不会改动（还有dynamic频繁改动；和stream每帧改动），DRAW表示将数据发送到GPU待绘制，即应用程序到GL（还有READ表数据将被客户端读取，GL到应用程序；和COPY表数据可用于绘制和读取，GL到GL）]
